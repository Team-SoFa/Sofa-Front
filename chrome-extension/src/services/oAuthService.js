...
import { get, post, put, del, tokenPost, refreshPost} from "./apiClient";
...
export const oAuth2RefreshPost = async () => {
  try {
    const response = await refreshPost("/oauth2/refresh", {}, {});
    console.log('oAuth2RefreshPost response:', response);  // 응답을 제대로 출력해보세요
    ...
    if (response) {
      ...
      }else return response // 토큰 저장
    } else {
      throw new Error('응답 데이터가 없습니다.');
    }
  } catch (error) {
    console.error("oAuth2RefreshPost failed:", error);
    throw error;  // 에러 발생 시 호출한 곳으로 에러 전달
  }
};